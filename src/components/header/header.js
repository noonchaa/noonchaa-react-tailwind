...
const Header = ({children,glass,centered,sticky,left,onClick}) => {
    return(
        <header className={`shadow ${glass?'bg-transparent':'bg-slate-50 dark:bg-slate-800'} ${sticky?'sticky top-0 z-50':'relative'}`}>
            <nav className={`container px-6 py-4 mx-auto relative`}>
                {centered?'':
                <div className={`md:hidden absolute top-4 z-50 ${left?'left-6':'right-6'}`} onClick={onClick}>
                ...
                <div className={`${centered?'flex justify-center items-center':'md:flex md:justify-between md:items-center'} ${left?'ml-6 md:ml-0':''}`}>
                {children}
                </div>
            </nav>
        </header>
